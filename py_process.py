...
separate processes to avoid the GIL. 采用多进程，防止GIL锁
  ...
  separate process."""
    ...
    def __getattr__(self, name):
        def call(*args):
            ...
            result = tf.py_func(py_call, (name,) + args, flat_dtypes,
                                name=name)
    ...
    def _worker_fn(self, type_, constructor_kwargs, in_):
        try:
            ...
            while True:
                ...
                method_name = serialized[0].decode()   # 进入pipeline以后变成字节码，出来需要解码
                inputs = serialized[1:]
...
class PyProcessHook(tf.train.SessionRunHook):
    ...
    def begin(self):  # 在创建Session之前调用，之后静态图生成，不能再被修改
    ...
    def end(self, session):  # 在Session结束时调用，被用于Hook想要执行最后的操作。
