...
VTraceFromDistributionReturns = collections.namedtuple(
    'VTraceFromDistributionReturns',
    ['vs', 'pg_advantages', 'rhos',
     'behaviour_action_distribution', 'target_action_distribution'])
...
def from_logits(
        ...
        discounts, rewards, values, bootstrap_value, flags,agent_name,ret_rms=None,   # values, bootstrap_value
...
def from_importance_weights(
        log_rhos, discounts, rewards, values, bootstrap_value, flags,agent_name,ret_rms=None,    # values, bootstrap_value
        clip_rho_threshold=1.0, clip_pg_rho_threshold=1.0,
    ...
    with tf.name_scope(name, values=[
        ...
        # popart
        if flags.reward_clipping == 'adaptive_normalisation':
            target_values=rewards + discounts * vs_t_plus_1
            old_mean, old_std = ret_rms.mean, ret_rms.std
            ret_rms.update(target_values)
            utils.setup_popart(ret_rms, old_mean, old_std, agent_name)

            pg_advantages = (
                    clipped_pg_rhos * (target_values - values))  # advantage
            ...
            pg_advantages = (
                    clipped_pg_rhos * (rewards + discounts * vs_t_plus_1 - values))  # advantage
...
def from_distribution(
        behaviour_policy_distribution, target_policy_distribution, actions,
        discounts, rewards, values, bootstrap_value,
        clip_rho_threshold=1.0, clip_pg_rho_threshold=1.0,
        name='vtrace_from_logits'):
    behaviour_policy_distribution = tf.convert_to_tensor(
        behaviour_policy_distribution, dtype=tf.float32)
    target_policy_distribution = tf.convert_to_tensor(
        target_policy_distribution, dtype=tf.float32)
    actions = tf.convert_to_tensor(actions, dtype=tf.float32)

    # Make sure tensor ranks are as expected.  保证相应维度准确
    # The rest will be checked by from_action_log_probs.
    behaviour_policy_distribution.shape.assert_has_rank(3)
    target_policy_distribution.shape.assert_has_rank(3)
    actions.shape.assert_has_rank(3)

    with tf.name_scope(name, values=[
        behaviour_policy_distribution, target_policy_distribution, actions,
        discounts, rewards, values, bootstrap_value]):
        rhos = target_policy_distribution / behaviour_policy_distribution
        vtrace_returns = from_importance_weights_distribution(  # pg_advantages, vs (length,batch)
            rhos=rhos,
            discounts=discounts,
            rewards=rewards,
            values=values,  # learner_outputs.baseline, (length,batch)
            bootstrap_value=bootstrap_value,
            clip_rho_threshold=clip_rho_threshold,  # 重要的clip，防止方差过大
            clip_pg_rho_threshold=clip_pg_rho_threshold)
        return VTraceFromDistributionReturns(
            rhos=rhos,  # (length,batch) log of rhos
            behaviour_action_distribution=behaviour_policy_distribution,  # (length,batch)
            target_action_distribution=target_policy_distribution,  # (length,batch)
            **vtrace_returns._asdict()  # pg_advantages, vs (as dict)
        )


def from_importance_weights_distribution(
    ...
    values = tf.convert_to_tensor(values, dtype=tf.float32)
    bootstrap_value = tf.convert_to_tensor(bootstrap_value, dtype=tf.float32)
    if clip_rho_threshold is not None:  # rho_t
        clip_rho_threshold = tf.convert_to_tensor(clip_rho_threshold,
                                                  dtype=tf.float32)
    if clip_pg_rho_threshold is not None:  # rho_s, rhos
        clip_pg_rho_threshold = tf.convert_to_tensor(clip_pg_rho_threshold,
                                                     dtype=tf.float32)

    # Make sure tensor ranks are consistent. (assert 是个好习惯）
    rho_rank = rhos.shape.ndims  # Usually 2. 二维：length, batch
    values.shape.assert_has_rank(rho_rank - 1)
    bootstrap_value.shape.assert_has_rank(rho_rank - 2)
    discounts.shape.assert_has_rank(rho_rank - 1)
    rewards.shape.assert_has_rank(rho_rank - 1)
    if clip_rho_threshold is not None:
        clip_rho_threshold.shape.assert_has_rank(0)
    if clip_pg_rho_threshold is not None:
        clip_pg_rho_threshold.shape.assert_has_rank(0)
    ...
    with tf.name_scope(name, values=[
        ...
        rhos = tf.reduce_mean(rhos, axis=-1)

        if clip_rho_threshold is not None:  # 1.0
            clipped_rhos = tf.minimum(clip_rho_threshold, rhos, name='clipped_rhos')  # rhos
        else:
            clipped_rhos = rhos

        cs = tf.minimum(1.0, rhos, name='cs')  # c
        # Append bootstrapped value to get [v1, ..., v_t+1]
        values_t_plus_1 = tf.concat(
            [values[1:], tf.expand_dims(bootstrap_value, 0)], axis=0)  # concat,变成后l个,(length,batch)

        # rhos * TD-error
        deltas = clipped_rhos * (rewards + discounts * values_t_plus_1 - values)

        # Note that all sequences are reversed, computation starts from the back. 按照指定维度反转
        sequences = (
            tf.reverse(discounts, axis=[0]),  # (length,batch)
            tf.reverse(cs, axis=[0]),  # (length,batch)
            tf.reverse(deltas, axis=[0]),  # (length,batch)
        )
        ...
        initial_values = tf.zeros_like(bootstrap_value)  # (batch, ) 初始化
        vs_minus_v_xs = tf.scan(  # 熟记公式,非常巧妙。reverse后公式累乘就对了，然后再转回来！！
            fn=scanfunc,
            elems=sequences,
            initializer=initial_values,
            parallel_iterations=1,
            back_prop=False,  # 不进行反向传播
            name='scan')
        # Reverse the results back to original order.
        vs_minus_v_xs = tf.reverse(vs_minus_v_xs, [0], name='vs_minus_v_xs')

        # Add V(x_s) to get v_s. 完整的Vtrace
        vs = tf.add(vs_minus_v_xs, values, name='vs')

        # Advantage for policy gradient.
        vs_t_plus_1 = tf.concat([  # (length,batch)
            vs[1:], tf.expand_dims(bootstrap_value, 0)], axis=0)
        if clip_pg_rho_threshold is not None:
            clipped_pg_rhos = tf.minimum(clip_pg_rho_threshold, rhos,
                                         name='clipped_pg_rhos')
        ...
        # Make sure no gradients backpropagated through the returned values.
        return VTraceReturns(vs=tf.stop_gradient(vs),  # 注意，不进行反向传播
                             pg_advantages=tf.stop_gradient(pg_advantages))
